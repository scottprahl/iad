...
{
    if (AGrid_N < AGrid_Cap)
        return;
    AGrid_Cap = (AGrid_Cap == 0) ? AGRID_INIT_CAP : AGrid_Cap * 2;
    AGrid_Cache = (agrid_entry_t *) realloc(AGrid_Cache, (size_t) AGrid_Cap * sizeof(agrid_entry_t));
    if (AGrid_Cache == NULL) {
...
static int agrid_find_entry(double a, double b, double g)
{
    int i;
    for (i = 0; i < AGrid_N; i++) {
        if (AGrid_Cache[i].a == a && AGrid_Cache[i].b == b && AGrid_Cache[i].g == g)
            return i;
    }
    return -1;
}
...
static int agrid_add_or_get(double a, double b, double g)
{
    ...
    AGrid_Cache[idx].b = b;
    AGrid_Cache[idx].g = g;
    abg_eval(a, b, g, &AGrid_Cache[idx].ur1, &AGrid_Cache[idx].ut1, &AGrid_Cache[idx].uru, &AGrid_Cache[idx].utu);
    AGrid_N++;
    return idx;
}
...
static void agrid_subdivide(double u0, double u1, double v0, double v1, int depth, int search)
{
    double um = 0.5 * (u0 + u1);
    double vm = 0.5 * (v0 + v1);
    double a00, b00, g00;
...
{
    if (!AGrid_Initialized || AGrid_N == 0)
        return 0;
    ...
    if (m.slab_cos_angle != AGrid_slab_cos_angle)
        return 0;
    if (m.slab_top_slide_index != AGrid_slab_n_top)
        return 0;
    if (m.slab_bottom_slide_index != AGrid_slab_n_bottom)
        return 0;
    if (r.slab.b_top_slide != AGrid_slab_b_top)
        return 0;
    if (r.slab.b_bottom_slide != AGrid_slab_b_bottom)
        return 0;

    if (m.num_spheres != AGrid_num_spheres)
        return 0;
    if (m.num_measures != AGrid_num_measures)
        return 0;
    if (m.num_measures == 3 && m.m_u != AGrid_m_u)
        return 0;

    if (r.search == FIND_AB && r.slab.g != AGrid_fixed_g)
        return 0;
    if (r.search == FIND_AG && r.slab.b != AGrid_fixed_b)
        return 0;
    if (r.search == FIND_BG) {
    ...
    for (i = 0; i < AGrid_N; i++) {
        ...
        if (Debug(DEBUG_GRID) && d < 0.15) {
            fprintf(stderr, "AGRID_ALL: a=%8.5f b=%9.5f g=%7.4f ur1=%8.5f ut1=%8.5f d=%9.6f\n",
                AGrid_Cache[i].a, AGrid_Cache[i].b, AGrid_Cache[i].g, AGrid_Cache[i].ur1, AGrid_Cache[i].ut1, d);
        }
        if (n_best < AGRID_CLOSURE_N) {
            ...
            int worst = 0;
            for (j = 1; j < n_best; j++)
                if (best_dist[j] > best_dist[worst])
                    worst = j;
...

    {
        ...
        if (n0 >= 2) {
            ...
            for (k = 1; k < n_sorted; k++) {
                ...
                l = k - 1;
                while (l >= 0 && sorted0[l] > tmp) {
                    sorted0[l + 1] = sorted0[l];
                    l--;
                }
                sorted0[l + 1] = tmp;
            }
        ...
        if (n1 >= 2) {
            ...
            for (k = 1; k < n_sorted; k++) {
                ...
                l = k - 1;
                while (l >= 0 && sorted1[l] > tmp) {
                    sorted1[l + 1] = sorted1[l];
                    l--;
                }
                sorted1[l + 1] = tmp;
            }
        ...
        for (k = 0; k < n0; k++) {
            for (l = 0; l < n1; l++) {
                ...
                d = abg_stored_distance(AGrid_Cache[idx].a,
                    AGrid_Cache[idx].b,
                    AGrid_Cache[idx].g,
                    AGrid_Cache[idx].ur1, AGrid_Cache[idx].ut1, AGrid_Cache[idx].uru, AGrid_Cache[idx].utu);
