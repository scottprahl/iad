...
static size_t lookup_slot_const(const Cache * c, double a, double b, double g, uint64_t h, int *found);

static size_t lookup_slot(Cache * c, double a, double b, double g, uint64_t h, int *found);
...
{
    if (!c)
        return -1;              /*release the old table to prevent a leak.                         */
    if (c->tbl) {
        free(c->tbl);
        c->tbl = NULL;
    }
...
int
cache_get_by_index(const Cache *c, size_t i,
...
static int key_equal(const CacheRecord *r, double a, double b, double g)
{
...
static size_t lookup_slot_const(const Cache *c, double a, double b, double g, uint64_t h, int *found)
{
    ...
    for (;;) {
        ...
        else if (s->hash == h && s->rec.a == a && s->rec.b == b && s->rec.g == g) {
            *found = 1;
            return pos;
        }
        pos = (pos + 1) & mask;
