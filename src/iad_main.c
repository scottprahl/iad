...
print_usage (void)
{
  ...
  fprintf (stderr,
	   "  -c #             measurements have unscattered light?\n");
  fprintf (stderr,
	   "  -C               do not correct for diffuse light loss\n");
  ...
  fprintf (stderr,
	   "  -G #             't' (one top) or 'b' (one bottom) slide\n");
...
main (int argc, char **argv)
{
  ...
  while ((c = my_getopt (argc, argv,
			 "?1:2:a:A:b:B:c:d:D:e:E:f:F:g:G:hi:n:N:M:o:p:q:r:R:S:t:T:u:vV:x:Xz"))
	 != EOF)
    {
      int n;
      char cc;
	...
	case 'G':
	  if (optarg[0] == '0')
	    ...
	    {
	      fprintf (stderr,
		       "Argument for -G option must be 't' (for top)");
  ...
  if ((cl_no_unscat == MR_IS_ONLY_RD)
      || (cl_no_unscat == NO_UNSCATTERED_LIGHT))
    m.sphere_with_rc = 0.0;

  if ((cl_no_unscat == MT_IS_ONLY_TD)
      || (cl_no_unscat == NO_UNSCATTERED_LIGHT))
