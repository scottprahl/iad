...
\author{
Scott Prahl\thanks{Oregon Tech, scott.prahl@oit.edu}
}
\date{May 2026}	
...
have made.  Upon matching, those optical properties should characterize 
your sample.
...
This document describes version 4.0.0 of the \iadprog{}.  The program is documented in the
...
For the first few months of 2024, I reviewed the entire program and
...
Inverse adding-doubling is a method for estimating the optical properties of
scattering and absorbing materials.  Reflection and transmission measurements,
typically made with an integrating sphere, are converted to the optical
properties of the sample (scattering, absorption, and anisotropy) using the
computer program \iadprog{}.
\iadprog{} has been extensively tested and validated for accuracy and
...
the appropriate header for your experiment and then append your
...
\end{center}
You will get back a file \texttt{data.txt} that contains a set of optical
properties---the scattering, absorption, and anisotropy coefficients that
correspond to each measurement set.

\iadprog{} finds the optical properties characterizing a sample by using
the measured total reflectance $M_R$, the measured total transmittance $M_T$,
and, when available, the measured unscattered transmittance $M_U$.
A set of optical properties is guessed and values for $M_R$ and $M_T$ are
calculated.  These calculated values are compared with the measured values.
If they match, then the optical properties for the sample have been found.
If they do not match, then a new set of optical properties is guessed and the
process is repeated.  As van de~Hulst pointed out, the approach is fairly
brute force.  Oh well, it works.
...
to determine one, two, or three optical properties.  \iadprog{} is intended to
be used from the command line.  The processed output file contains a summary of
the experimental conditions as well as all the calculated optical
properties.\footnote{The optical properties are readily graphed using
...
\begin{enumerate}
\item
the distribution of light is independent of time,
\item
the sample has homogeneous optical properties,
\item
the sample geometry is an infinite plane-parallel slab,
\item
...
\begin{enumerate}
\item
it works for any combination of optical properties,
\item
it takes into account the interactions of a sample sandwiched between glass slides,
\item
it incorporates the effects of the integrating spheres on the measured values,
\item
it accounts for lost light (light that is not collected by the spheres), and
\item
it has a reasonable trade-off between accuracy and speed.
\end{enumerate}

\subsection{Background}

\subsubsection*{The graduate student days}
\iadprog{} started its life as an inverse diffusion equation program. Steve Jacques (then at the Wellman
...
simplex method of Nelder and Mead.  I used the inverse $\delta$-Eddington
model in my 1982 thesis, supervised by Dr.\ Jon Valvano and the late Dr.\ A.\ J.\ Welch.
...
Lake City, with generous support from Dr.\ Richard Straight.  The starting routine was
...
At the same time, I became fascinated with the exceptional typesetting capabilities of \TeX{}---the same system used to compose this document.  Given that \iadprog{} is mathematically complex, I wanted aesthetically pleasing and clear typesetting for my comments.  This led me to adopt \texttt{CWEB}\footnote{\href{https://github.com/ascherer/cwebbin}{\texttt{CWEB} system}}, which allows for the creation of elegant documentation, albeit at the expense of a preprocessing step.
...
The stored-grid approach addresses the initial guess problem.  This is, once again, a
brute-force calculation done before the inversion process starts.  For example, when the albedo $a$ and optical thickness
...
included two thicknesses of identical polyurethane material as well as phantoms with the
...
\item using a reflectance standard whose reflectance was closer to that of the sample, and
...
they suffer from having a non-linear response to the presence of a sample.  For
...
\iadprog{} has had code to account for the integrating sphere effect since
...
Incorporating these new formulas into the \iadprog{} code also gave me the chance
...
We now know that it is quite important to know the sphere
...
some light reflected by the sample will not be collected (because it emerges on
...
edge of the sample port; when this light re-emerges from the front of
...
A Monte Carlo program is used to estimate the amount of light lost for this
set of optical properties.  This lost light is incorporated back into the next
inverse adding-doubling estimate of the optical properties.  The process is
repeated until the optical properties stabilize.  Most experimental situations
require only a few Monte Carlo iterations.  Furthermore, when analyzed with the
new \iadprog{} that accounted for lost light, all the earlier results for the
contract phantoms that we made were now consistent!

\subsubsection*{User Issues 3}

\clearpage
\section{How IAD works}

This section describes how \iadprog{} turns measured reflection and transmission
values into optical properties.  The adding-doubling calculation itself is
treated here as a forward calculation: given an albedo $a$, optical thickness
$b$, anisotropy $g$, boundary indices, slide properties, and illumination
geometry, it returns the slab reflection and transmission values needed by the
inverse program.  The inverse part of \iadprog{} is the machinery that chooses
trial optical properties, predicts what the experiment would measure for those
properties, and decides whether the prediction is close enough to the data.

\subsection{The main data flow}

For each wavelength, \iadprog{} works with two central records.  The measurement
record describes the experiment: measured $M_R$, measured $M_T$, optional
$M_U$, sample and slide geometry, integrating-sphere geometry, unscattered-light
collection fractions, beam diameter, and the current estimates of light lost
outside the spheres.  The inversion record describes the analysis: the current
slab optical properties, user-specified constraints, the selected search mode,
the distance metric, tolerances, iteration counts, and the final status.

The inversion entry point validates the measurements, chooses a search mode
when the user has not explicitly specified one, and then calls the appropriate
one- or two-parameter search.  Each trial follows the same path:
\begin{enumerate}
\item start from a trial optical state $(a,b,g)$,
\item run the forward adding-doubling and boundary calculations to obtain
      ideal slab reflection and transmission values,
\item convert those ideal slab values into predicted instrument measurements
      by applying unscattered-light, sphere, and lost-light corrections,
\item calculate a scalar distance between predicted and measured values, and
\item let the optimizer choose the next trial state.
\end{enumerate}
After a successful inversion, the reported $\mu_a$, $\mu_s$, and $\mu_s'$ are
derived from the best values of $a$, $b$, $g$, and the physical sample
thickness.  In this notation, $b$ is the total optical depth and $a$ is the
fraction of that optical depth due to scattering.

\subsection{Choosing the unknowns}

The program does not always solve for the same variables.  The search mode is
chosen from the independent measurements and the constraints supplied by the
user.  One-parameter searches solve for one of $a$, $b$, $g$, $\mu_a d$, or
$\mu_s d$ when the remaining information is fixed or implied.  Two-parameter
searches solve for pairs such as $(a,b)$, $(a,g)$, $(b,g)$,
$(\mu_a d,g)$, or $(\mu_s d,g)$.  The common case with measured $M_R$ and
$M_T$ and a fixed or default $g$ is an $(a,b)$ search.

The search-selection code first counts available measurements and constraints.
It then removes measurements that do not contain useful diffuse information in
the current limiting case.  If the number of constraints plus measurements is
larger than the three optical properties can support, \iadprog{} warns that the
problem is over-constrained, but it still tries to choose a reasonable search.

\subsection{Forward prediction}

Every optimizer callback updates the trial optical properties and asks for the
distance.  The distance calculation first obtains the four ideal slab values
\[
  \mathrm{UR1},\quad \mathrm{UT1},\quad \mathrm{URU},\quad \mathrm{UTU}.
\]
The first two are total reflection and transmission for direct illumination;
the second two are the corresponding quantities for diffuse illumination.
The code also calculates the unscattered reflected and transmitted parts,
called $R_u$ and $T_u$, from specular propagation through the boundaries,
sample, and slides.

These ideal slab quantities are not yet the measurements.  They are passed to
the measurement model, which accounts for what the instrument actually collects.
This separation is important: adding-doubling predicts what an infinite slab
does, while the \iadprog{} measurement model predicts what the integrating
sphere experiment reports.

\subsection{Integrating-sphere corrections}

Integrating spheres change the relationship between slab reflectance and the
normalized measurements $M_R$ and $M_T$.  Before the sphere formulas are
applied, \iadprog{} forms corrected slab values.  Diffuse values are reduced by
diffuse lost light,
\[
  \mathrm{URU}_\mathit{calc}=\mathrm{URU}-\mathrm{URU}_\mathit{lost}, \qquad
  \mathrm{UTU}_\mathit{calc}=\mathrm{UTU}-\mathrm{UTU}_\mathit{lost}.
\]
Direct values are reduced both by intentionally uncollected unscattered light
and by scattered light lost outside the sphere ports,
\[
  \mathrm{UR1}_\mathit{calc}=\mathrm{UR1}-(1-f_{R_u})R_u-\mathrm{UR1}_\mathit{lost},
\]
\[
  \mathrm{UT1}_\mathit{calc}=\mathrm{UT1}-(1-f_{T_u})T_u-\mathrm{UT1}_\mathit{lost}.
\]
Here $f_{R_u}$ and $f_{T_u}$ are the fractions of unscattered reflection and
transmission included in the measured totals.

With no spheres, the predicted measurements are simply the corrected direct
reflection and transmission.  With one sphere, \iadprog{} models reflection
and transmission measurements separately, including wall reflectivity, sample,
detector, and third-port areas, baffles, standards, entrance-port effects,
unscattered-light collection, and sphere gain.  With two spheres, the
double-sphere formulas account for light exchanged between the reflection and
transmission spheres before detection.

Sphere correction is part of every distance evaluation.  The optimizer is
therefore fitting sphere-normalized measurements, not raw slab values.

\subsection{The adaptive grid}

Two-parameter inverse searches are nonlinear and a poor starting point can lead
to slow convergence or to the wrong local region.  For this reason \iadprog{}
builds an adaptive forward grid before launching the local optimizer.  The grid
is used for the main two-parameter searches $(a,b)$, $(a,g)$, and $(b,g)$.

The grid is built in transformed coordinates.  The albedo and anisotropy axes
use nonlinear maps that place more samples near sensitive boundaries, while the
optical-thickness axis is logarithmic because useful $b$ values span many
orders of magnitude.  Each grid entry stores the physical $(a,b,g)$ and the raw
adding-doubling values $\mathrm{UR1}$, $\mathrm{UT1}$, $\mathrm{URU}$, and
$\mathrm{UTU}$.
The final distance is not cached.  Instead, distances are recalculated when
candidate guesses are requested, so current sphere settings and current
lost-light estimates are always reflected.

The grid is rebuilt only when the physical context changes: the search mode,
sample or slide indices, slide thicknesses, illumination angle, number of
spheres, number of measurements, $M_U$ when present, or the fixed axis for the
active search.  Lost-light terms alone do not invalidate the grid because the
raw adding-doubling values can be reused.

The adaptive subdivision criterion is based on sphere-corrected
$(M_R,M_T)$ space.  A cell is refined when the value calculated at its center
differs too much from bilinear interpolation of its corners.  This puts more
forward calculations where the measurement response is changing rapidly,
instead of spending a fixed rectangular grid everywhere.

After the grid has been built, the best candidates are ranked using the same
measurement-space distance used by the optimizer.  The program also tries a
small local closure search by taking promising axis values, adding midpoints,
and evaluating their cross-combinations.  This helps find good starts when the
best point lies between diagonal grid samples.

\subsection{Seeding the simplex}

For two-parameter searches, the best grid or previous-result candidate becomes
the first vertex of the Nelder-Mead simplex.  The other vertices are formed by
small positive steps along the active coordinates.  The search is bounded in
physical coordinates: $0\le a\le1$, $b>0$, and
$-\mathtt{MAX\_ABS\_G}\le g\le\mathtt{MAX\_ABS\_G}$.

Under ordinary inversion, the default step is five percent of the current
coordinate, with a small floor when the coordinate is zero.  During Monte Carlo
re-inversion, the code supplies smaller hot-start steps because the previous
solution should already be close to the new solution after the lost-light
estimate has changed.  The grid gives the search a global starting region; the
small physical simplex gives the local optimizer a stable shape near that
region.

One-parameter searches do not use a simplex.  They use bracketing and Brent
minimization, with transformed coordinates where needed to keep the result in
the physical range.

\subsection{Lost light}

Adding-doubling is one-dimensional and assumes an infinite lateral slab.  Real
samples are finite.  Scattered photons can leave through the sample edge or
emerge outside a finite sphere port, reducing the light that the detector sees.
\iadprog{} estimates this finite-sample correction with an internal Monte Carlo
calculation.

The Monte Carlo calculation uses the current optical properties, sample and
slide indices, sample and slide thicknesses, beam diameter, illumination angle,
and sphere-port diameters inferred from the port areas.  It returns lost-light
fractions for direct reflection and transmission, and, for substitution sphere
measurements, diffuse reflection and transmission.

The command-line program feeds this result back into the inversion as a
fixed-point iteration:
\begin{enumerate}
\item run the ordinary sphere-corrected inversion with zero lost light,
\item run Monte Carlo for the current optical properties,
\item compare the new lost-light estimates with the stored estimates,
\item update the stored lost light by a relaxation factor rather than taking
      the full jump,
\item re-run the inversion using the updated lost-light values, and
\item repeat until the optical coefficients and lost-light estimates stabilize
      or the Monte Carlo iteration limit is reached.
\end{enumerate}
The number of photons is adapted during this process.  Early iterations can use
fewer photons to move cheaply; later iterations use more photons to reduce
Monte Carlo noise.

This feedback is needed because lost light depends on the optical properties,
but the optical properties inferred from $M_R$ and $M_T$ depend on the amount
of lost light.  The iterative loop solves these coupled estimates together.
\iadprog{} also checks whether the estimated lost light exceeds what is
physically compatible with the measurements and can scale the lost-light terms
down when necessary.

\subsection{What is minimized}

\iadprog{} does not minimize error in $a$, $b$, $g$, $\mu_a$, or $\mu_s'$.
It minimizes a scalar distance between the measured instrument values and the
predicted instrument values:
\[
  D = f(M_{R,\mathit{calc}}-M_{R,\mathit{meas}},
        M_{T,\mathit{calc}}-M_{T,\mathit{meas}}).
\]
The predicted values already include boundary handling, unscattered-light
collection choices, sphere gain, standards, ports, and current lost-light
estimates.

The default metric is a scaled $L^2$ distance.  Each residual is divided by
the larger of the measured value and 0.01, and then squared.  For
two-parameter searches, transmission is included and reflection is included
unless the albedo is fixed at zero.  For one-parameter searches, transmission
is used when a transmission measurement exists; otherwise reflection is used.
Older relative and absolute metrics are also available through the command-line
options.

Thus, success means success in corrected measurement space.  A small final
distance means that the final optical properties reproduce the measured
$M_R$ and $M_T$ under the specified instrument model.  It does not prove that
the assumed sphere geometry, standards, sample thickness, or fixed anisotropy
are correct.

\subsection{Success criteria}

The immediate success criterion is that the final distance is smaller than the
requested tolerance.  The default inversion tolerance is $10^{-4}$.  If the
optimizer reaches the iteration limit before satisfying this tolerance, the row
is marked as not found.  A later check can still accept a boundary case if the
final distance is already below tolerance.

After inversion, the measurements are checked again against physical and
instrument-specific limits.  When the data violate these limits, \iadprog{}
reports the more specific data-quality error rather than only reporting a
generic convergence failure.

When Monte Carlo lost-light correction is enabled, success also requires each
re-inversion to converge and the derived $\mu_a$ and $\mu_s'$ to stop changing
by more than the Monte Carlo tolerance.  If a Monte Carlo re-inversion fails,
the program preserves the most specific error code available.
...
The source code, release archives, and Windows binaries are available from the
\texttt{iad} project page

\begin{center}
\url{https://github.com/scottprahl/iad}
\end{center}

For most users the easiest path is to download the latest release archive from
the releases page.  The source archive is named like
\texttt{iad-\textit{version}.zip}.  The Windows archive is named like
\texttt{iad-win-\textit{version}.zip} and includes pre-built
\texttt{ad.exe} and \texttt{iad.exe} executables.  If you want the current
development version instead of a release, clone the repository with
\texttt{git}.

\begin{verbatim}
prompt> git clone https://github.com/scottprahl/iad.git
prompt> cd iad
\end{verbatim}

On Windows, the pre-built executables can be run from \texttt{cmd.exe} or
PowerShell without compiling anything.  On macOS and Linux, building from
source is usually just a normal \texttt{make} build.
...
The generated \texttt{.c} and \texttt{.h} files are included in the repository
and release archives.  This means that ordinary users do not need
\texttt{CWEB} just to compile \texttt{ad} and \iadprog{}.  You need
\texttt{CWEB} only if you edit the \texttt{.w} files or want to regenerate the
woven source listings.

\subsubsection{Building}

This code has been compiled and tested on macOS and Linux systems, including
x86, Apple silicon, and Raspberry Pi class ARM machines.  A C compiler and
\texttt{make} are required.  On macOS these are provided by the Xcode command
line tools; on Linux they are usually provided by the system compiler package
such as \texttt{build-essential}.

To compile both command-line programs from a source archive,
\begin{verbatim}
prompt> unzip iad-version.zip
prompt> cd iad-version
prompt> make
\end{verbatim}
...
prompt> make
\end{verbatim}
This produces two executable files in the top-level directory:
\texttt{ad}, the forward adding-doubling calculator, and \iadprog{}, the
inverse adding-doubling program.

Several more specific build targets are available:
\begin{center}
\begin{tabular}{lp{7cm}}
\texttt{make ad}          & build only the forward program \texttt{ad}\\
\texttt{make iad}         & build only the inverse program \iadprog{}\\
\texttt{make executables} & build \texttt{ad}, \iadprog{}, and developer test programs\\
\texttt{make lib}         & build \texttt{libiad} and the public \texttt{libiad.h} header\\
\texttt{make docs}        & rebuild the manual and CWEB source listings\\
\texttt{make clean}       & remove compiled binaries and intermediate files\\
\end{tabular}
\end{center}

Most users only need \texttt{make}.  Rebuilding the documentation or
regenerating generated source files requires extra developer tools, including
\texttt{perl}, \pdflatex{}, \texttt{bibtex}, \texttt{CWEB}, and GNU
\texttt{indent}.

\subsubsection{Installing}

The programs can be run directly from the build directory:
\begin{verbatim}
prompt> ./iad -r 0.3 -t 0.1
prompt> ./ad -a 0.9 -b 1 -g 0.8
\end{verbatim}

To install the programs into a system location, use
\begin{verbatim}
prompt> sudo make install
\end{verbatim}
By default this copies \texttt{ad} and \iadprog{} into \texttt{/usr/local/bin}.
The installation prefix can be changed by overriding \texttt{DESTDIR}; for
example,
\begin{verbatim}
prompt> make DESTDIR=$HOME/.local install
\end{verbatim}
installs the executables in \texttt{\$HOME/.local/bin}.  Make sure that
directory is on your shell path if you want to run \iadprog{} without typing
its full path.

The library interface is optional.  If you need \texttt{libiad}, build and
install it with
\begin{verbatim}
prompt> make lib
prompt> sudo make install-lib
\end{verbatim}
The default library extension is chosen near the top of the Makefile.  On
macOS it is usually \texttt{.dylib}; on Linux you'll want to set
\texttt{LIB\_EXT=.so} before building the library.

\subsubsection{Testing}

Once the code has been compiled, the quickest check is
\begin{verbatim}
...
prompt> make test
\end{verbatim}
The complete validation targets are described in the next section.

\clearpage

\section{Validation}

The repository includes command-line regression tests, batch tests over
\texttt{.rxt} files, and a few lower-level executable tests.  These tests are
not a proof that every possible experiment is correct, but they do exercise the
main pathways through \iadprog{}: command-line parsing, inverse searches,
forward calculations, sphere corrections, glass-slide boundary handling,
unscattered light handling, Monte Carlo lost-light correction, and file
processing.

Most tests use the shell scripts in \texttt{tests/cli}.  The scripts run the
program in a temporary directory, extract numeric output, and compare it against
stored reference values with a small tolerance.  The batch tests instead verify
that each \texttt{.rxt} file can be processed and that an output file is
created.

\subsection{Primary test targets}

\begin{description}
\item[\texttt{make veryshorttest}]
Builds \iadprog{} and runs the smallest deterministic command-line suite.  This
is the best installation smoke test.  It covers one-measurement, two-measurement,
and three-measurement inversions; no-sphere operation; a minimal one-sphere
case with Monte Carlo disabled; and the basic search paths used when $M_R$,
$M_T$, and $M_U$ are supplied from the command line.

\item[\texttt{make test} or \texttt{make shorttest}]
Builds \iadprog{} and \texttt{ad}, then runs the normal deterministic CLI
regression suite.  This includes all \texttt{veryshorttest} cases plus forward
mode \texttt{-z}, no-sphere file processing, one-sphere and two-sphere
deterministic sphere corrections, multiple input-file workflows, option
validation, and the basic \texttt{ad} forward-calculation checks.  Use this
target after ordinary code changes.

\item[\texttt{make longtest}]
Builds \iadprog{} and \texttt{mc\_lost}, processes every \texttt{.rxt} file
under \texttt{tests/rxt} with Monte Carlo disabled, and then runs the Monte
Carlo slide tests.  The batch pass is a broad compatibility test for the file
parser, header interpretation, wavelength-row processing, output-file creation,
and the many historical data-file formats represented in the test directory.
Everything here is deterministic, but it takes far longer than
\texttt{make test}.

\item[\texttt{make test0}]
Runs the batch file workflow only for \texttt{tests/rxt/0\_sphere}.  These
fixtures exercise inverse calculations when no integrating sphere correction is
applied.  They cover command-file inputs for simple reflectance-only,
reflectance/transmittance, and constrained optical-property searches.

\item[\texttt{make test1}]
Runs the batch file workflow for \texttt{tests/rxt/1\_sphere}.  These fixtures
exercise single-sphere reflectance and transmittance correction, sphere header
fields, wall reflectance, baffle choices, calibration standards, wavelength
ranges, and a wide collection of historical one-sphere data files.  Some cases
also exercise lost-light handling unless the target-specific options disable it.

\item[\texttt{make test2}]
Runs the batch file workflow for \texttt{tests/rxt/2\_sphere}.  These fixtures
exercise double-sphere substitution measurements, simultaneous $M_R$ and $M_T$
geometry, double-sphere gain calculations, and the two-sphere normalization used
for files such as \texttt{double.rxt}.

\item[\texttt{make test1\_nomc}]
Runs the one-sphere batch tests with \texttt{-M 0}.  This isolates the
deterministic single-sphere gain and normalization code from Monte Carlo
lost-light updates.  It is useful when checking changes to sphere correction,
input parsing, or search logic without introducing random Monte Carlo variation.

\item[\texttt{make test2\_nomc}]
Runs the two-sphere batch tests with \texttt{-M 0}.  This isolates deterministic
double-sphere normalization and search behavior from lost-light updates.
\end{description}

\subsection{What the short test suite covers}

The \texttt{make test} target is the most useful day-to-day validation target
because it combines focused numeric regressions with file-workflow checks.
Internally it runs the following scripts:

\begin{description}
\item[\texttt{test\_iad\_basic.sh}]
Checks command-line inversions from direct measurements.  It covers
reflectance-only, reflectance/transmittance, and reflectance/transmittance/
unscattered-transmittance cases; index mismatch; top, bottom, and paired glass
slides; slide absorption; fixed $g$, albedo, optical thickness, absorption, and
scattering constraints; and oblique incidence.

\item[\texttt{test\_iad\_forward.sh}]
Checks forward mode using \texttt{-z}.  It verifies total reflectance, total
transmittance, unscattered transmittance, oblique illumination, sample and slide
indices, slide configuration, quadrature selection, and quiet output with
\texttt{-V 0}.

\item[\texttt{test\_iad\_no\_sphere.sh}]
Checks file-based operation when \texttt{-S 0} is used.  This exercises the
\texttt{.rxt} parser, default output-file naming, explicit output naming, and
the no-sphere inverse path.

\item[\texttt{test\_iad\_one\_sphere\_no\_mc.sh}]
Checks deterministic one-sphere calculations with \texttt{-M 0}.  It covers
reflection-only, $M_R/M_T$, and $M_R/M_T/M_U$ inversions, plus a fully baffled
one-sphere geometry.

\item[\texttt{test\_iad\_two\_sphere\_no\_mc.sh}]
Checks deterministic double-sphere calculations with \texttt{-M 0}.  It covers
reflection-only, $M_R/M_T$, $M_R/M_T/M_U$, and lower-reflectance/lower-
transmittance cases using explicit reflection and transmission sphere
parameters.

\item[\texttt{test\_iad\_files.sh}]
Checks ordinary file workflows.  It verifies default \texttt{.txt} output,
\texttt{-o} output naming, multiple input files on one command line, globbed
inputs, and ignoring non-\texttt{.rxt} files.

\item[\texttt{test\_iad\_options.sh}]
Checks option parsing and error handling.  It covers help and version output,
standard reflectance/transmittance options \texttt{-R} and \texttt{-T},
unscattered collection fractions \texttt{-c} and \texttt{-C}, wall-first light
\texttt{-f}, fixed-property constraints, power-law scattering with wavelength,
wall-reflectance shorthands \texttt{-w} and \texttt{-W}, search override
\texttt{-s}, baffle selection \texttt{-H}, grid output \texttt{-J}, wavelength
filtering \texttt{-l}, and expected failures for invalid options or missing
files.

\item[\texttt{test\_iad\_slides.sh}]
Checks the forward reflectance and transmittance for every \texttt{-G} slide
configuration, with and without absorbing slides.  No spheres are used, so no
Monte Carlo lost-light estimate enters and every value is deterministic.
Besides the tabulated numbers it asserts the relationships behind them: that
\texttt{-G 0} is unaffected by \texttt{-E}, that \texttt{-G n} and \texttt{-G f}
reproduce \texttt{-G t} and \texttt{-G b} exactly, that a slide on the near face
is not the same experiment as one on the far face, and that one absorbing slide
passes more light than two and less than none.

\item[\texttt{test\_ez\_rt\_callers.sh}]
A source check rather than a run.  The \texttt{ez\_RT} family takes indices of
refraction but no slide optical depths and sets both to zero internally, so
those routines treat the slides as clear.  This test fails if any file that can
see slide absorption starts calling them, which would silently drop the
absorption.  Only \texttt{ad\_layers\_test} and \texttt{mc\_lost\_main} are
allowed to call them, and it also checks that \texttt{mc\_lost\_main} still
fixes its slide absorption at zero.

\item[\texttt{test\_ad\_basic.sh}]
Checks the forward adding-doubling executable \texttt{ad}.  This is not an
\iadprog{} inversion test, but it validates the forward solver that
\iadprog{} depends on.  It covers normal illumination, oblique illumination,
glass slides, help/version output, and file input/output for \texttt{ad}.
\end{description}

\subsection{What the long test suite covers}

Beyond processing every \texttt{.rxt} file, \texttt{make longtest} runs one
script that is too slow for the short suite because it launches millions of
photons.

\begin{description}
\item[\texttt{test\_mc\_slides.sh}]
Checks that the Monte Carlo lost-light estimate treats the two slides
separately.  It verifies that a slide on either face changes the answer, that
the loss is ordered no slides $<$ one slide $<$ two slides, that a top slide
spreads reflected light more while a bottom slide spreads transmitted light
more, and that the Monte Carlo totals agree with adding-doubling when the port
is wide enough that nothing is lost.  It also checks that \texttt{-G n} and
\texttt{-G f} leave the reflection losses alone while changing the transmission
losses, since lost light is not reciprocal.

Because this is the only test covering how the Monte Carlo handles the two
slides, a change to \texttt{mc\_lost} that \texttt{make test} passes may still
be wrong.  Run \texttt{make longtest} after touching the lost-light code.
\end{description}

\subsection{Monte Carlo and helper executable targets}

\begin{description}
\item[\texttt{make mc\_test}]
Builds and runs the internal Monte Carlo test executable.  This checks the
lost-light machinery used by \iadprog{} when \texttt{-M} is non-zero.

\item[\texttt{make mc\_lost\_test}]
Builds \texttt{mc\_lost} and runs several command-line lost-light cases.  These
exercise photon launch settings, normal incidence, refractive-index mismatch,
slide-index mismatch, and anisotropic scattering in the stand-alone lost-light
driver.

\item[\texttt{make layer\_test} and \texttt{make layertest}]
Build and run the layered-slab test executable.  This checks the lower-level
adding-doubling layer combination code.  It is mostly a forward-solver test, but
that solver is the numerical base used by \iadprog{}.

\item[\texttt{make cone\_test}]
Builds and runs the conical-illumination test executable.  This checks support
code for finite cone illumination in the forward adding-doubling routines.

\item[\texttt{make oblique\_test}]
Builds and runs the oblique-incidence test executable.  This checks angular
quadrature and boundary behavior for non-normal illumination.

\item[\texttt{make wintest}]
Cross-builds the Windows executables and runs tests under \texttt{wine}.  It
checks that the Windows command-line programs behave like the native build,
including basic CLI tests and batch processing of \texttt{.rxt} files.
\end{description}

\subsection{Choosing a validation target}

During normal installation, run \texttt{make veryshorttest}.  During ordinary
development, run \texttt{make test}.  Before a release, run \texttt{make test},
\texttt{make longtest}, the sphere-specific batch targets, and the Monte Carlo
helper targets.  Use \texttt{make wintest} when preparing or checking a Windows
release.
...
The following tables show that these results remain valid when experiments
...
A sample has three optical properties: scattering, absorption, and scattering anisotropy.
To determine all three, we need three measurements. Typically these are
total reflection, total transmission, and unscattered transmission. In practice,  
you often do not need $g$ and therefor can measure total reflection and total transmission,
to determine only $\mu_a$ and $\mu_s$.  Sometimes the sample is really thick and some
scattering or absorption value must be assumed.now you need to decide how you will actually make the measurements. The table
below relates the basic premise that you can't get something for nothing; it gives
the necessary measurements for each set of optical properties.
...
\begin{tabular}{c|c|p{7cm}}
 ...
 Property              & Values             & \\
\hline
 $\mu_a$, $\mu_s$, $g$ & $M_R$, $M_T$, $M_U$& \\
 $\mu_a$, $\mu_s'$     & $M_R$, $M_T$       & Default assumption is $g=0$\\
 $\mu_a$               & $M_R$              & Default assumption is $g=0$, $\mu_s=1$/mm.\\
\end{tabular}
\end{center}
\end{table}
...
make the easy things easy, while hard things remain possible.

\begin{center}
\fbox{\it I have made a bunch of decisions for you.}
\end{center}

For example, \iadprog{} assumes that the \textit{total} reflection $M_R$
and \textit{total} transmission $M_T$ are measured: $M_R$ and $M_T$ include
the unscattered reflection or unscattered transmission respectively.  
Another reasonable alterative is to allow the specular (unscattered) reflection from the sample
to exit entrance port so $M_R$ is only diffuse light (use the \texttt{-c 0} command
line option.%
\footnote{Since the unscattered reflectance does not interact with the sample (much)
it does not provide any information about the scattering or
absorption properties.  If you go this route, then make sure all the unscattered
light leaves --- any of the unscattered light will collected will increase 
...
better.  I decided that, in general, both approaches are bad ideas.
...
diverge appreciably.  Unfortunately, illumination is nearly always
...
set of measurements.  However, we tried a couple of other schemes, such as
...
the background problem is much smaller.\footnote{Using two
...
understand.  For example, the header
...
The derivations and examples in this manual assume that there is a baffle
between the sample and the detector.  As I said before, this is still the
recommended geometry because it suppresses the direct sample-to-detector
path on the first bounce.  However, the current \iadprog{} implementations
can model all four reflection/transmission baffle combinations with the
\texttt{-H 0}, \texttt{-H 1}, \texttt{-H 2}, and \texttt{-H 3} options.
The fully baffled case (\texttt{-H 3}) remains the reference configuration
for the equations and examples in this manual.

\subsection{Which reflectance standard?}

You will need some reflectance standards for the reflection measurement.  Common NIST (formerly NBS)
...
When such a standard is used, all the sample measurements
...
The index of refraction of polyurethane is $\sim$1.5 and therefore the
...
\cite{sumpner92,ulbricht00,taylor20,jacquez55,goebel67,pickering92}.  Good old Ulbricht
...
and its wall reflectance (typically $\sim$98\%).  In the validated derivations
that follow, it is assumed that the baffle (inside the sphere) is located
...
some fraction of light directly propagates from the sample to the detector, while
the rest of the light bounces around in the sphere.  The current code can still
model these alternative baffle states with \texttt{-H}, but the gain expressions
must then be adjusted accordingly.  For matching the equations, notebook checks,
and examples in this manual, it is much better to keep the baffle and avoid the
whole mess.  
...
on the sample port, you will no longer have a perfect sphere.  A rule of
...
\iadprog{} \textit{ makes no corrections for the integrating spheres.}  In this
mode, lost-light correction is also undefined, so Monte Carlo lost-light
iterations are skipped even if \texttt{-M} is non-zero.  Good
...
is significant.
\item
You want to evaluate how much effect the sphere corrections are having on
...
your data.}
\item
You don't know the sphere wall reflectivity.  If you want robust measurements,
there is some merit in determining the reflectivity as outlined in this document.
...
affects the results of the sphere making the measurements.
...
used at a time for all measurements.  For one-sphere work, the default
normalization is the substitution method, but comparison-mode normalization is
also available with \texttt{-X}.  The baffle state for the reflection and
transmission measurements is controlled by \texttt{-H 0..3}.
...
sample has dynamically changing optical properties, then it is critical
...
One does not want to squish one side of the sample so that barely adequate
optical contact is reached on both sides.  The second drawback is that it is
a pain in the neck to make the calibration experiments.  The command-line
programs treat the two-sphere case as a substitution-mode model only; the
\texttt{-X} dual-beam option cannot be combined with \texttt{-S 2}.  The
double-sphere equations in this manual correspond to the validated baffled
configuration, while the code can still swap in the appropriate gain expressions
for other \texttt{-H} baffle settings.
...
lamp variation with wavelength, detector changes, or anything that affects the
...
in these two modified images~\cite{wendlandt66}.  In both examples,
...
light loss is not an issue in the idealized normalization argument.  In the current
\iadprog{} implementation, lost-light estimation for dual-beam data is still treated
as a separate modeling step: \texttt{-S 0 -X} disables both sphere and lost-light
corrections, while \texttt{-S 1 -X} uses the beam and sphere geometry for the
Monte Carlo lost-light estimate.

Anyhow, if you are using a dual-beam system and your system automatically
...
How does one analyze these in practice with IAD?  Well, the simplest way is to
use the \texttt{-X} option\footnote{The `X' is for crossed beams.} and specify
the sphere parameters that you used.  In the current programs, \texttt{-X} may be
combined only with \texttt{-S 0} or \texttt{-S 1}; \texttt{-S 2 -X} is rejected.
In addition, \texttt{-X} assumes that no light hits the reflection sphere wall
first, so \texttt{-f} must remain zero.  If you choose \texttt{-S 0}, then no
sphere or lost-light model is applied.  If you choose \texttt{-S 1} and provide
the sphere geometry, then the comparison-mode normalization is used while the
beam and port geometry can still be used for the lost-light estimate.

\subsection{How should the sample be prepared?}

There are a whole host of experimental problems that plague measurements
...
measurement and flaky optical models.  Since, presumably, the adding-doubling
method is \textit{not} flaky, you are saved from the second source of error.  
But I can do nothing to save you from poor experimental technique
...
done on the changes in optical properties that take place when a tissue
is removed.  There has been minor work done on the changes in optical
properties with heating, but nothing that definitively says that the
...
match the quantities calculated by the code.  Water, saline, preferably
...
tissue.  It is important to store the tissue in airtight containers
...
``Relatively little'' is complex and depends on other things.  In general,
it is hard to predict without modeling carefully.  Before the Monte Carlo
correction was added to this code, all losses would be attributed to absorption.  Consequently, any absorption coefficients generated for this sample would be too large.  The ramifications now are less dire: lost light takes longer to analyze because
the amount of lost light must be continually recalculated.  

\subsubsection{Variability}
From spot to spot on one sample and from sample to sample, optical properties vary.  Make enough
...
Clearly, neither is a good example of the in vivo situation.  If
samples have no blood, then the optical properties of the sample itself are
being measured.  The contribution from blood can be added later.
...
evidence for this has been from muscle tissue.  Other tissues like aorta and
dermis are different and are not as susceptible to freezing artifact.

\subsubsection{Interference}  
Serious pain in the wazoo.  The interference usually comes from multiple
internal reflections from the glass
slides that sandwich the tissue sample.  When a laser beam strikes the
...
reflectance.  One would expect this to be a negligible effect, but in practice
it can be nearly 10\%.  It is easily demonstrated by placing a microscope
...
consequently will not interfere.  Thick plates are not ideal because they
displace the integrating spheres and consequently allow some diffuse light to
be lost before it can enter the integrating sphere.

A final possibility is to use optically flat plates.  Interference will be
...
Note that this figure shows the fully baffled reference geometry with the baffle between the sample and
the detector.  The transmission sketch also shows the common case in which the third port is covered
so that light that does not interact with the sample bounces around in the sphere.  Finally,
...
\end{equation}
These are the same substitution-mode double-sphere formulas validated in the
double-sphere derivation notebook and implemented in both the CWEB and Python
programs.  In the code, the directly measured $\rdirect$, $\tdirect$,
$\rdiffuse$, and $\tdiffuse$ terms are first corrected for any uncollected
unscattered component and, when enabled, for lost light, and only then are
they normalized with the double-sphere gains.
...
1          # Number of spheres used at a time to make a measurement

           # Sphere configuration for reflectance measurements
203.2      # [mm] Sphere Diameter
25.4       # [mm] Sample Port Diameter
12.7       # [mm] Light Entrance Port Diameter
1.00       # [mm] Detector Port Diameter
0.96       # Reflectance of the sphere wall

           # Sphere configuration for transmittance measurements
203.2      # [mm] Sphere Diameter
25.4       # [mm] Sample Port Diameter
0.00       # [mm] Light Exit Port Diameter (closed)
1.00       # [mm] Detector Port Diameter
0.96       # Reflectance of the sphere wall
...
The third entry of the transmittance block is the exit port rather than an
entrance port, because the unscattered beam leaves the sphere through it
instead of entering.  It may be set to zero, which means the port is covered
and the whole direct beam lands on the sphere wall.  If it is left open it
must be at least as wide as the beam, since otherwise only part of the direct
beam would reach the standard sitting in the port; \iadprog{} refuses such a
geometry rather than calculate with it.  The sample port size should
...
sphere loses light only through the detector port and the sample port.\footnote[1]{Of
...
spheres and therefore the exit port size in the transmittance sphere might
...
The two sphere blocks are always both present, even when only one sphere is
used.  ``One sphere'' means one sphere at a time: the reflectance is measured
with the sample on one sphere and the transmittance with the sample on another,
and those two need not be the same sphere or have the same sample port.  Only
when two spheres are used simultaneously does the sample sit in a single hole
facing both, and then the sample port diameter must be the same in both blocks
for the sphere corrections to be right.  When no spheres are used, neither
block has any effect and both are marked \texttt{(unused)}.
...
0          # Number of spheres used at a time to make a measurement

           # Sphere configuration for reflectance measurements (unused)
203.2      # [mm] Sphere Diameter
25.4       # [mm] Sample Port Diameter
12.7       # [mm] Light Entrance Port Diameter
...
0.96       # Reflectance of the sphere wall
           ...
           # Sphere configuration for transmittance measurements (unused)
203.2      # [mm] Sphere Diameter
25.4       # [mm] Sample Port Diameter
12.7       # [mm] Light Exit Port Diameter
12.7       # [mm] Detector Port Diameter
0.96       # Reflectance of the sphere wall
...
The CWEB \texttt{iad} program is currently the reference implementation for the
full command-line interface documented in this section.  The Python
\texttt{iadcommand.py} interface already supports the common measurement and
constraint switches and is being brought into parity with the remaining
advanced options documented here.  Where the two implementations differ today,
the switch semantics described below should be read as the canonical \texttt{iad}
behavior.
...
\begin{description}
    ...
    \item[-A \#] Assume this absorption coefficient for all calculations.\\[1mm]
                 ...
                 will be found assuming that $g=0$, unless \texttt{-g} is also specified.
    ...
    \item[-C \#] The fraction of unscattered transmitted light in your transmission \texttt{MT} measurement.\\[1mm]
                 This is a number from 0 to 1.  If the third port is closed or covered so that all of the unscattered
                 transmitted light is returned to the sphere, then use \texttt{-C 1}.  If the third port is open so that
                 the unscattered transmitted light leaves the sphere, then use \texttt{-C 0}.  Intermediate values may be
                 used to model partial collection of the unscattered transmitted light.  In the current one-sphere
                 transmission model, any non-zero value of \texttt{-C} implies that the third port is filled by the
                 calibration reflector during the sample measurement, while \texttt{-C 0} implies an open third port.\\[1mm]
    ...
    \item[-e \#] The error tolerance for success.\\[1mm]
                 ...
                 \textit{The default value is 0.0001, \texttt{-e 0.0001}.}
    ...
    \item[-f \#] The fraction of incident light that hits the sphere wall first.\\[1mm]
                 ...
                 This option cannot be combined with the dual-beam \texttt{-X} configuration.\\[1mm]
    ...
    \item[-g \#] Assume this value for the scattering anisotropy for all calculations.\\[1mm]
                 ...
                 $0$ (isotropic), to $+1$ (completely forward scattering).  Values exactly equal to $-1$ or $+1$ are
                 accepted on the command line, but are moved slightly inward internally to avoid singular calculations.\\[1mm]
    ...
    \item[-G \#] Specify the slide/boundary configuration.\\[1mm]
                 Legal values are \texttt{0}, \texttt{2}, \texttt{t}, \texttt{b},
                 \texttt{n}, and \texttt{f}.  The value \texttt{0} means no slides,
                 \texttt{2} means identical slides on both sides, \texttt{t} means
                 a single slide on the top surface (hit by the incident beam first),
                 and \texttt{b} means a single slide on the bottom surface.  The
                 values \texttt{n} and \texttt{f} describe a single slide by its
                 position relative to the sphere that collects the light: near or far.\\[1mm]
                 This option should be used together with \texttt{-N} to specify the
                 slide index and, when needed, \texttt{-D} and \texttt{-E} to set the
                 slide thickness and absorption.

                 The \texttt{n} and \texttt{f} forms describe a sample that is turned
                 over between the two measurements, as the figure below shows, so the
                 slide keeps its position relative to the sphere.  Their reflectances
                 and transmittances are identical to those of \texttt{t} and
                 \texttt{b}: turning the sample over cannot change the total
                 transmittance, which is the same whichever face the light enters, and
                 the reflectance is calculated for the face the beam actually strikes
                 in the reflection measurement.  The distinction still matters for the
                 Monte Carlo estimate of lost light, which is not reciprocal, since it
                 depends on which face carries the glass that spreads the light
                 sideways.
                 \begin{center}
                 \includegraphics[width=\textwidth]{option-G.jpg}
                 \end{center}
    \item[-h] display help
    \item[-H \#] Specify the integrating-sphere baffle configuration.\\[1mm]
                 Use \texttt{-H 0} for no baffle in either sphere, \texttt{-H 1}
                 for a baffle in the reflection sphere only, \texttt{-H 2} for a
                 baffle in the transmission sphere only, and \texttt{-H 3} for
                 baffles in both spheres.  The derivations and most examples in
                 this manual use the fully baffled reference configuration
                 \texttt{-H 3}.\\[1mm]
                 This is an expert option that changes the gain expressions used
                 for the sphere corrections.
    \item[-i \#] Specify the incident angle in degrees.\\[1mm]
                 This is primarily used when the unscattered reflected light is
                 collected because the beam is intentionally oblique.  If an oblique
                 angle is specified, then the quadrature order should be a multiple
                 of 12 rather than just a multiple of 4.
    \item[-j \#] Constrain the reduced scattering coefficient $\mu_s'$.\\[1mm]
                 This is the reduced-scattering analogue of \texttt{-F}.  When
                 \texttt{-g} is also fixed, the program converts this to
                 $\mu_s=\mu_s'/(1-g)$ internally.  This is mainly useful for
                 expert, single-parameter inversions.
    \item[-J] Generate a grid after the inverse calculation.\\[1mm]
                 This is a developer/debugging option that writes a grid of
                 calculated values after the inversion, which is useful when
                 inspecting the search surface or preparing plots.
    \item[-l '\# \#'] Restrict processing to a wavelength interval.\\[1mm]
                 The argument contains the lower and upper wavelength limits.
                 For example, \texttt{-l '400 700'} processes only wavelengths
                 between 400 and 700 in the input file.
    \item[-L \#] Explicitly specify the wavelength $\lambda$.\\[1mm]
                 This is mainly useful when the program is run entirely from the
                 command line rather than from a file.  The value is recorded as
                 the wavelength for the calculation and is also used by wavelength-
                 dependent constraints such as the power-law version of \texttt{-F}.
    \item[-M \#] Set the maximum number of Monte Carlo lost-light iterations.\\[1mm]
                 Legal values are integers from \texttt{0} to \texttt{50}.  The default is \texttt{19}.
                 Use \texttt{-M 0} to skip the lost-light Monte Carlo update entirely.
    \item[-n \#] Specify the index of refraction of the slab.\\[1mm]
                 The value must be between \texttt{0.1} and \texttt{10}.  The default is \texttt{1.0}.
    \item[-N \#] Specify the index of refraction of the slides.\\[1mm]
                 The value must be between \texttt{0.1} and \texttt{10}.  The same value is applied to
                 both slides unless the slide configuration removes one of them.
    \item[-o filename] Explicitly specify the output filename.\\[1mm]
                 This may be used for a single input file or a command-line calculation, but it cannot
                 be used when one command processes multiple \texttt{.rxt} files.
    \item[-p \#] Set the Monte Carlo photon count or time limit.\\[1mm]
                 A positive value is the number of photons.  A negative value is interpreted as the
                 maximum time, in milliseconds, for one Monte Carlo lost-light calculation.
    \item[-q \#] Set the number of quadrature points.\\[1mm]
                 The value must be a multiple of \texttt{4}.  If \texttt{-i} is used for oblique
                 incidence, it must be a multiple of \texttt{12}.  The default is \texttt{8}.
    \item[-r \#] Total reflection measurement, $M_R$.\\[1mm]
                 This enables command-line inversion and must be between \texttt{0} and \texttt{1}.
    \item[-R \#] Actual reflectance for the 100\% reflection standard.\\[1mm]
                 The value must be between \texttt{0} and \texttt{1}.  If only \texttt{-R} is supplied,
                 the same value is also used as the transmission standard unless \texttt{-T} overrides it.
    \item[-s \#] Override the automatic inverse-search choice.\\[1mm]
                 Normally the program determines the appropriate one- or two-
                 parameter search from the available measurements and constraints.
                 The \texttt{-s} option is an expert override for that choice and
                 is mainly useful for debugging or reproducing earlier runs.
    \item[-S \#] Number of integrating spheres used.\\[1mm]
                 Legal values are \texttt{0}, \texttt{1}, and \texttt{2}.  With \texttt{-S 0},
                 no sphere correction or lost-light Monte Carlo correction is applied.
    \item[-t \#] Total transmission measurement, $M_T$.\\[1mm]
                 This enables command-line inversion and must be between \texttt{0} and \texttt{1}.
    \item[-T \#] Actual transmittance for the 100\% transmission standard.\\[1mm]
                 The value must be between \texttt{0} and \texttt{1}.  If \texttt{-R} is not supplied,
                 this value is also used for the reflection standard.
    \item[-u \#] Unscattered transmission measurement, $M_U$.\\[1mm]
                 This enables command-line inversion and must be between \texttt{0} and \texttt{1}.
    \item[-v] version information
    \item[-V 0 | 1 | 2] verbosity level.\\[1mm]
                 Use \texttt{-V 0} for quiet output, \texttt{-V 1} for progress
                 reporting while the file is processed, and \texttt{-V 2} for the
                 default, more verbose behavior.  These are the canonical CWEB
                 \texttt{iad} semantics; the Python CLI is being aligned to match them.
    \item[-w \#] Wall reflectivity for the reflection sphere.\\[1mm]
                 This is a convenient way to override just the reflection-sphere
                 wall reflectance when the full \texttt{-1} sphere tuple is not
                 being used.  If \texttt{-1} is supplied, then \texttt{-w} is
                 redundant and the full sphere specification should be preferred.
    \item[-W \#] Wall reflectivity for the transmission sphere.\\[1mm]
                 This is the transmission-sphere analogue of \texttt{-w}.  If
                 \texttt{-1} or \texttt{-2} already provides the sphere geometry
                 and wall reflectivity, then \texttt{-W} should be omitted.
    \item[-x \#] Set the debugging bit mask.\\[1mm]
                 This is a developer-facing option that enables different classes
                 of intermediate output such as grid selection, Monte Carlo
                 iterations, sphere gains, or the full search trace.
    \item[-X] Use the dual-beam comparison configuration.\\[1mm]
                 This is allowed only with zero or one sphere and cannot be combined with a nonzero
                 \texttt{-f} wall-first fraction.
    \item[-z] Do a forward calculation.\\[1mm]
                 In this mode \texttt{iad} reports the reflection and transmission predicted by the
                 specified optical properties instead of inverting measurements.
\end{description}
...
\texttt{ -z                  }& do a forward calculation                   \\
\end{tabular}
\end{center}
...
The \texttt{-z} switch changes \iadprog{} from an inverse program into a
forward calculator.  Instead of finding optical properties from measured
values, the program uses the optical properties and experimental geometry
specified on the command line and prints the predicted reflection,
transmission, and, when spheres are active, the predicted measured sphere
signals.  For example,
\begin{verbatim}
                prompt> iad -z -a 0.9 -b 1 -g 0.8
\end{verbatim}
uses albedo $a=0.9$, optical thickness $b=1$, and anisotropy $g=0.8$.
Forward mode is often the easiest way to check the meaning of a geometry or
constraint option before using it in an inversion.
...
\texttt{ -M \#               }& maximum number of Monte Carlo lost-light iterations \\
\texttt{ -s \#               }& expert override for the inverse search type \\
\texttt{ -J                  }& write a grid after the inverse calculation \\
\texttt{ -l '\# \#'          }& restrict input-file processing to a wavelength range \\
...
as necessary (up to 19) to ensure that the predicted $\mu_a$ and $\mu_s'$ have
stabilized between successive MC/IAD updates.  The current implementation also
continues iterating while the directly lost reflected or transmitted components
are still changing appreciably, so that the direct-loss estimate settles before
the loop stops.  Now the number of Monte Carlo iterations is \textit{not} the same as
the number of photons used in the Monte Carlo calculation of lost light.
Instead, the \texttt{-M \#} option controls the maximum number of times that the
lost light calculation is done.  If you specify \texttt{-e}, then the same
numeric tolerance is used for these successive-coefficient checks.  When
\texttt{-S 0} is used, the lost-light model is not active and \texttt{-M}
has no practical effect.  The command-line parser accepts values from
\texttt{0} through \texttt{50}.
...
Use\footnote{\texttt{double.rxt} is found in \texttt{tests/rxt/2\_sphere/}.}
\begin{verbatim}
                prompt> iad -M 0 tests/rxt/2_sphere/double.rxt
\end{verbatim}
...
\begin{verbatim}
                prompt> iad tests/rxt/2_sphere/double.rxt
\end{verbatim}
...
the command
\begin{verbatim}
                prompt> iad -p 10000 tests/rxt/2_sphere/double.rxt
\end{verbatim}
...
\begin{verbatim}
                prompt> iad -p -100 tests/rxt/2_sphere/double.rxt
\end{verbatim}
In the canonical \texttt{iad} command-line interface, a negative value is
interpreted as a time limit in milliseconds for a single Monte Carlo iteration,
rather than as a photon count.
...
\begin{verbatim}
                prompt> iad -q 4 tests/rxt/2_sphere/double.rxt
\end{verbatim}
takes only 0.1 seconds.  While
\begin{verbatim}
                prompt> iad -q 16 tests/rxt/2_sphere/double.rxt
\end{verbatim}
takes 1.3 seconds.
When the incidence angle is oblique, as specified with \texttt{-i}, the
quadrature order should be a multiple of 12.

\subsubsection{Restricting the Wavelength Range}

The \texttt{-l '\# \#'} option restricts an input-file calculation to a
wavelength interval.  The argument contains the lower and upper wavelength
limits, separated by whitespace and therefore normally quoted.  For example,
\begin{verbatim}
                prompt> iad -l '500 600' sample.rxt
\end{verbatim}
processes only rows whose wavelength is between 500 and 600 in the units used
by the file.  This option is ignored for a single command-line calculation
that does not read wavelength rows from an \texttt{.rxt} file.

\subsubsection{Writing a Grid}

The \texttt{-J} option asks \iadprog{} to write a small grid of calculated
$M_R$ and $M_T$ values after the inverse calculation.  The grid is written next
to the normal output file with a \texttt{.grid} suffix.  This is mainly a
developer and plotting aid for inspecting the search surface; it does not
change the fitted optical properties.

\subsubsection{Overriding the Search Type}

The \texttt{-s \#} option overrides the search type that the program would
normally choose automatically.  This is an expert/debugging option that is most
useful when reproducing a previous run or when you want to inspect a specific
inversion path.  The default is the automatic mode \texttt{-s 4}, which lets
\iadprog{} choose the search based on the available measurements and the
constraints that have been specified.  The supported search codes are
\texttt{0}=\texttt{FIND\_A}, \texttt{1}=\texttt{FIND\_B},
\texttt{2}=\texttt{FIND\_AB}, \texttt{3}=\texttt{FIND\_AG},
\texttt{4}=\texttt{FIND\_AUTO}, \texttt{5}=\texttt{FIND\_BG},
\texttt{6}=\texttt{FIND\_BaG}, \texttt{7}=\texttt{FIND\_BsG},
\texttt{8}=\texttt{FIND\_Ba}, \texttt{9}=\texttt{FIND\_Bs},
\texttt{10}=\texttt{FIND\_G}, \texttt{11}=\texttt{FIND\_B\_WITH\_NO\_ABSORPTION},
and \texttt{12}=\texttt{FIND\_B\_WITH\_NO\_SCATTERING}.  In normal use, it is
better to let the program choose this automatically.
...
\texttt{ -j \#               }& fixed reduced scattering coefficient       \\
...
calculation.  The default value is 0.  The parser accepts values from $-1$ to
$+1$; exact endpoint values are moved slightly inward internally to keep the
Henyey-Greenstein phase function finite.  This option is particularly useful when you have only made two measurements,
...
\subsubsection{Constraining the Reduced Scattering Coefficient}

The \texttt{-j \#} option fixes the reduced scattering coefficient
$\mu_s'=\mu_s(1-g)$.  This is mainly useful when you know $\mu_s'$ more
reliably than $\mu_s$, or when you want to force a one-parameter search while
holding the reduced scattering fixed.  The value must be non-negative.  If you also specify \texttt{-g}, then
the program converts the constraint to $\mu_s=\mu_s'/(1-g)$ internally.
...
\end{verbatim}
The albedo must be between 0 and 1.
...
worry about specular reflection at boundaries.  The value must be
non-negative.
...
handy to specify it directly.  The value is in mm$^{-1}$ and must be
non-negative.  Oddly the constraint that I use most
...
in a while it comes in handy.  The value is in mm$^{-1}$ and must be
non-negative.  The power-law form
\texttt{-F 'P lambda0 mus0 gamma'} is also accepted and sets
$\mu_s(\lambda)=\mu_s(\lambda_0)(\lambda/\lambda_0)^\gamma$.
...
\texttt{ -R \#               }& actual reflectance for 100\% measurement   \\
\texttt{ -T \#               }& actual transmittance for 100\% measurement \\
\texttt{ -c \#               }& fraction of unscattered reflection in $M_R$\\
\texttt{ -C \#               }& fraction of unscattered transmission in $M_T$\\
\texttt{ -f \#               }& fraction of incident light that hits wall first\\[3mm]
\texttt{ -i \#               }& specify incidence angle                    \\
\texttt{ -L \#               }& specify wavelength                         \\
\texttt{ -n \#               }& specify index of refraction of sample      \\
\texttt{ -N \#               }& specify index of refraction of slides      \\
\texttt{ -G \#               }& specify slide or boundary configuration    \\
\texttt{ -d \#               }& physical thickness of the sample           \\
\texttt{ -D \#               }& physical thickness of the slides           \\
\texttt{ -S \#               }& number of spheres used                     \\
\texttt{ -X                  }& dual-beam comparison configuration         \\
\texttt{ -H \#               }& specify baffle configuration               \\
\texttt{ -w \#               }& reflection sphere wall reflectivity        \\
\texttt{ -W \#               }& transmission sphere wall reflectivity      \\
...
The \texttt{-R \#} and \texttt{-T \#} options specify the actual reflectance
and transmittance of the standards that define the 100\% measurement for the
reflection and transmission channels.  Both values must be between 0 and 1.
If \texttt{-R} is supplied and \texttt{-T} is omitted, then the reflection
standard value is also used for the transmission standard.  If \texttt{-T} is
supplied and \texttt{-R} is omitted, then the transmission standard value is
also used for the reflection standard.

\subsubsection{Unscattered Collection Fractions}
The \texttt{-c \#} and \texttt{-C \#} options specify how much of the
unscattered reflected and transmitted light is included in the measured
$M_R$ and $M_T$ values.  Both values must be between 0 and 1.  Use
\texttt{-c 0} when specular reflection leaves through the entrance port and is
not measured; use \texttt{-c 1} when the experimental geometry collects it.
Likewise, \texttt{-C 0} describes an open transmission third port that does not
return the unscattered beam to the sphere, while \texttt{-C 1} describes full
collection.

\subsubsection{Wall-First Reflection}
The \texttt{-f \#} option specifies the fraction of incident light that hits
the reflection-sphere wall before reaching the sample.  The value must be
between 0 and 1.  This can model an overfilled sample port or a beam that is
larger than the sample.  It is not supported with the dual-beam comparison
configuration selected by \texttt{-X}.

\subsubsection{The Incidence Angle}
The \texttt{-i \#} option allows you to specify the angle of incidence in
degrees.  This is most often used when all of the unscattered reflected light
is deliberately collected by using oblique incidence.  When \texttt{-i} is
used, the quadrature order should be a multiple of 12.

\subsubsection{The Wavelength}
The \texttt{-L \#} option explicitly specifies the wavelength for a
command-line calculation.  This is primarily useful when no input file is
being read and when wavelength-dependent constraints, such as the power-law
form of \texttt{-F}, are being used.

\subsubsection{The Index of Refraction of the Slab}
The \texttt{-n \#} option allows you to specify the index of refraction of
the sample.  This number must be between 0.1 and 10.  The default is 1.0.

\subsubsection{The Index of Refraction of the Glass Slides}
The \texttt{-N \#} option allows you to specify the index of refraction of
the glass slides.  This number must be between 0.1 and 10.  The default is 1.0.
The same value is applied to both slides; the slide configuration selected by
\texttt{-G} can then remove the top or bottom slide as needed.

\subsubsection{The Slide or Boundary Configuration}
The \texttt{-G \#} option specifies how many slides are present and where a
single slide is located.  Use \texttt{-G 0} for no slides, \texttt{-G 2} for
identical slides on both sides, \texttt{-G t} when the incident beam hits the
top slide first, and \texttt{-G b} when it hits the bottom slide first.  The
values \texttt{-G n} and \texttt{-G f} describe a single slide by whether it is
near or far from the collecting sphere.  This option is normally paired with
\texttt{-N}, and often with \texttt{-D} and \texttt{-E}, to provide the slide
index, thickness, and absorption.

\subsubsection{The Physical Thickness of the Sample}
The \texttt{-d \#} option allows you to specify the physical thickness of
the sample.  This number must be non-negative and in millimeters.  The default is 1.0\,mm.

\subsubsection{The Physical Thickness of the Glass Slides}
The \texttt{-D \#} option allows you to specify the thickness of
the glass slides.  This number must be non-negative.  The default is 0.0\,mm
...
This number must be 0, 1, or 2.  The default is zero.  With \texttt{-S 0},
the program applies no sphere correction and no lost-light model.  With
\texttt{-S 1}, the program uses the single-sphere model and may be combined
with \texttt{-X} for comparison-mode normalization.  With \texttt{-S 2}, the
program uses the double-sphere substitution model; \texttt{-X} is not allowed.

\subsubsection{The Dual-Beam Configuration}
The \texttt{-X} option selects the comparison, or dual-beam, configuration.
It can be used only with zero or one integrating sphere.  If one sphere is used,
the sphere port sizes must be known because the lost-light calculation still
needs the sample-port and beam geometry.  The option cannot be used with
\texttt{-S 2}, and it cannot be combined with a nonzero \texttt{-f} value.

\subsubsection{The Baffle Configuration}
The \texttt{-H \#} option chooses which integrating spheres include an internal
baffle between the sample and the detector.  Use \texttt{-H 0} for no baffles,
\texttt{-H 1} for a reflection-sphere baffle only, \texttt{-H 2} for a
transmission-sphere baffle only, and \texttt{-H 3} for baffles in both spheres.
The fully baffled case is the recommended reference geometry and is the one used
for most derivations in this manual.

\subsubsection{Sphere Wall Reflectivity Overrides}
The \texttt{-w \#} and \texttt{-W \#} options set the wall reflectivity of the
reflection and transmission spheres respectively.  These are convenient shorthands
when you do not want to specify the full \texttt{-1} or \texttt{-2} sphere tuples.
If the full sphere definitions are already being supplied, then these options are
redundant and should be omitted.  The command-line program treats \texttt{-w}
as an error when \texttt{-1} is also supplied, and treats \texttt{-W} as an
error when explicit sphere geometry is supplied.

\subsubsection{The Reflection Sphere Properties}
The \texttt{-1 '\# \# \# \# \#'} option allows you to specify the properties
of the reflection sphere. The five numbers (four diameters and the wall reflectance)
...
The units should be millimeters.  \texttt{-1} also initializes the transmission-sphere geometry to the same
values.  Use \texttt{-2} when the transmission sphere is different.

\subsubsection{The Transmission Sphere Properties}
The \texttt{-2 '\# \# \# \# \#'} option allows you to specify the properties
of the transmission sphere. The five numbers (four diameters and the wall reflectance)
must be surrounded by quotes.  The 
five numbers must appear in this order (sphere diameter, sample port diameter,
third port diameter, detector port diameter, and sphere wall reflectance).
...
the \texttt{.rxt} is replaced by \texttt{.txt}).  The \texttt{-o} option may
appear after a single input filename, but it cannot be used when one command
processes multiple \texttt{.rxt} files.  For example,
\begin{verbatim}
                prompt> iad -o foobar tests/rxt/2_sphere/double.rxt
\end{verbatim}
...
The canonical \texttt{iad} command-line interface uses \texttt{-V 0},
\texttt{-V 1}, and \texttt{-V 2} to control verbosity.  The Python CLI is
being aligned to use the same convention.  The \texttt{-V 0} option allows you
to eliminate the extra terminal output from the \iadprog{} program.  This is not
...
output\footnote{\texttt{vio\_A.rxt} is located in \texttt{tests/rxt/1\_sphere/}.}
\begin{verbatim}
                prompt> iad tests/rxt/1_sphere/vio_A.rxt
                prompt> iad -V 1 tests/rxt/1_sphere/vio_A.rxt
\end{verbatim}
...
\begin{verbatim}
                prompt> iad -V 0 tests/rxt/1_sphere/vio_A.rxt
\end{verbatim}
is used, then nothing is displayed on the screen until the program finishes.

\texttt{-V 2} is the default.  It is just like \texttt{-V 1} except it
prints a message after processing the file.

\subsubsection{Showing Intermediate Calculations}

The debugging mask documented here is the full CWEB \texttt{iad} mask.  The
Python interface is being brought into parity with these same flags.
...
\texttt{ -x 16               }& show starting guess / best-grid choice         \\
\texttt{ -x 32               }& show information about the search choice       \\
\texttt{ -x 64               }& show grid calculations                         \\
\texttt{ -x 128              }& show sphere gain calculations                  \\
\texttt{ -x 256              }& show every calculation                         \\
\texttt{ -x 512              }& show Monte Carlo internals                     \\
\texttt{ -x 1023             }& all of the above                               \\
\end{tabular}
\end{center}
...
\begin{verbatim}
                prompt> iad -x 1 tests/rxt/2_sphere/double.rxt
\end{verbatim}
...
via the the glass slides of the holder, as in Figure~\ref{glass_slide}.  The circle on the top represents light incident at
...
\begin{figure}[tp]
\includegraphics[scale=0.945]{glass_slide.pdf}
