...
Incorporating Monte Carlo into the \iadprog{} led to the next major release ---
version 3.0.0 in May of 2006.
...
In Windows you can start \texttt{Cmd.exe} by
...
Navigate to the directory containing \texttt{iad.exe}.

Alternatively, you can add the directory containing \texttt{iad.exe} to your PATH environment variable.%
\footnote{When started from a shortcut, \texttt{Cmd.exe} inherits the environment variables set in `My Computer \textgreater Properties \textgreater Advanced \textgreater Environment Variables`.}

Navigate to the directory containing your data files and test the executable by running:
\begin{verbatim}
    prompt> iad -v
\end{verbatim}
...
\begin{description}
    ...
    \item[-F \#] Constrain the scattering coefficient.\\[1mm]
                 The value for this option has three possible cases.  First, when the argument is a simple number
                 \texttt{-F 0.5} then the scattering coefficient is kept fixed at 0.5\,mm$^{-1}$.\\[1mm]  
                 The second
                 option is when the value is '$P$ $\lambda_0$ $\mu_{s0}$ $\gamma$' where the assumed scattering
                 coefficient depends on wavelength according to $\mu_{s0} (\lambda/\lambda_0)^\gamma$.  The 
                 value $\mu_{s0}$ is the assumed scattering coefficient at $\lambda_0$.  So the 
                 option \texttt{-F 'P 1000 1.5 -1.3'} will use the wavelength $\lambda$ to generate the scattering
                 coefficient $\mu_s = 1.5 (\lambda/1000)^{-1.3}$.\\[1mm]  
                 The third option is when the value is '$R$ $\lambda_0$ $\mu_{s0}$ $\gamma$' where the assumed scattering
                 coefficient depends on wavelength according to $\mu_{s0} (\lambda/\lambda_0)^\gamma$.  The 
                 value $\mu_{s0}$ is the assumed scattering coefficient at $\lambda_0$.  So the 
                 option \texttt{-F 'R 1000 1.5 -1.3'} will use the wavelength $\lambda$ to generate the scattering
                 coefficient $\mu_s' = 1.5 (\lambda/1000)^{-1.3}$.
    ...
    \item[-g \#] Assume this value for the scattering anisotropy for all calculations\\[1mm]
                 ...
                 The program assumes a Henyey-Greenstein scattering phase function and $g$ is the cosine of the average
